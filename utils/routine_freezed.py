from utils import data_loader, converter_freezed, model_implementation, model_utils, reproducibility, evaluation
import os
import numpy as np
...
def leave_one_out(dialogue_ids, df, project_dir, run_path, config, config_params, text_model, audio_model, sample_rate):
    ...
    encoded_X, encoded_y = converter.encode_data(df, text_model)
    ...
    for dialogue_id in dialogue_ids:
        ...
        train_indexes = df.loc[df['Dialogue ID'] != dialogue_id].index.values
        test_indexes = df.loc[df['Dialogue ID'] == dialogue_id].index.values

        encoded_Xtrain, y_train = encoded_X[train_indexes], encoded_y[train_indexes]
        encoded_Xtest, y_test = encoded_X[test_indexes], encoded_y[test_indexes]

        test_snippet = df.loc[df['Dialogue ID'] == dialogue_id]['Snippet'].values
        ...
        # mark the 'Split' column as 'Test' for the current for test indexes
        df.loc[test_indexes, 'Split'] = 'Test'
        # mark the 'Split' column as 'Train' for the remaining dialogue IDs
        df.loc[train_indexes, 'Split'] = 'Train'
        ...
        train_sentence_snippet = encoded_Xtrain
        test_sentence_snippet = encoded_Xtest
        ...
        if config != 'text_only':
            encoded_audio_train, max_frame_len = converter_freezed.prepare_audio_data(train_audio_files, model_type='embedding', audio_model=audio_model,
        ...
        if config != 'text_only':
            encoded_audio_test = converter_freezed.prepare_audio_data(test_audio_files, model_type='embedding', audio_model=audio_model, audio_model_sample_rate=sample_rate)

        # get the number of labels
        num_labels = data_loader.get_num_labels(y_train)


        # Instatiate the model

        if config !='text_only':
            model = model_implementation.build_model(num_labels,
                                                        embedding_dim=encoded_Xtrain.shape[-1],
        ...
        test_sentence_snippet = df.loc[df['Dialogue ID'] == dialogue_id]['SentenceSnippet'].values
        # save predictions
...
def leave_one_out_baselines(dialogue_ids, df, project_dir, run_path, config, config_params, model):
    ...
    for dialogue_id in dialogue_ids:
        ...
        ytrain = converter_freezed.encode_labels(ytrain)
        ytest = converter_freezed.encode_labels(ytest)
