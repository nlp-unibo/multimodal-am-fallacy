...
def prepare_text_data(X,
    ...
    # store inputs_ids_not_padded to calculate the maxSentenceLen
    data = tokenizer([x.strip() for x in X], truncation=True)
    input_ids = data['input_ids']
    attention_mask = data['attention_mask']

    sample_idx = 0
    encoded_data = []
    with tqdm(total=int(np.ceil(len(input_ids) // batch_size))) as pbar:
        while sample_idx < len(input_ids):
            batch_input_ids = input_ids[sample_idx:sample_idx + batch_size]
            max_length = max([len(seq) for seq in batch_input_ids])

            batch_input_ids = [seq + [tokenizer.pad_token_id] * (max_length - len(seq)) for seq in batch_input_ids]
            batch_input_ids = np.array(batch_input_ids)

            batch_attention_mask = [seq + [0] * (max_length - len(seq)) for seq in
                                    attention_mask[sample_idx:sample_idx + batch_size]]
            ...
            encoded_data.append(encoder_model(batch_data)[-1].numpy())
            sample_idx += batch_size

            pbar.update(1)

    encoded_data = np.concatenate(encoded_data)
    return encoded_data, y_enc


def encode_data(df, model): 

    X = df['SentenceSnippet'].values
    y = df['Fallacy'].values

    if model == 'bert':
        tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")
        encoder_model = TFBertModel.from_pretrained('bert-base-uncased', trainable=False)
    elif model == 'roberta':
        tokenizer = RobertaTokenizerFast.from_pretrained("roberta-base")
        encoder_model = TFRobertaModel.from_pretrained('roberta-base', trainable=False)
    elif model == 'sbert':
    ...
    encoded_X, encoded_y = prepare_text_data(X=X,
                                             y=y,
                                             tokenizer=tokenizer,
